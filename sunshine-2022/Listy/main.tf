...
resource "google_service_account" "listy-frontend" {
  account_id   = "listy-frontend"
  display_name = "Listy frontend service account"
}

resource "google_cloudfunctions_function_iam_member" "listy-frontend" {
  project        = google_cloudfunctions_function.listy.project
  region         = google_cloudfunctions_function.listy.region
  cloud_function = google_cloudfunctions_function.listy.name
  role           = "roles/cloudfunctions.invoker"
  member         = "serviceAccount:${google_service_account.listy-frontend.email}"
}

resource "google_cloud_run_service" "listy-frontend" {
  name     = "listy-frontend"
  ...
  template {
    spec {
      containers {
        image = "gcr.io/sunshine-2022-challenges/listy-frontend"
      }
      service_account_name = google_service_account.listy-frontend.email
    }
  }
  traffic {
    percent         = 100
